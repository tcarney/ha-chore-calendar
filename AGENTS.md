...
- **Repository:** tcarney/ha-chore-calendar

**Key directories:**

- `custom_components/chore_calendar/` - Main integration code
...
- Integration logs: `custom_components.chore_calendar: debug` in `config/configuration.yaml`
...
- Use the domain `chore_calendar` for all DOMAIN references
- Prefix all integration-specific classes with `ChoreCalendar`
- Use "Chore Calendar" as the display title
...
- Inherit from platform base + `ChoreCalendarEntity`
...
- `tests/` mirrors `custom_components/chore_calendar/` structure
