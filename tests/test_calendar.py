...
@pytest.mark.usefixtures("enable_custom_integrations")
async def test_get_events_this_skip_leaves_hole(hass, config_entry):
    """A THIS-skipped occurrence is a hole in the expansion; the following
    occurrences are unaffected (recurrence model §3 checkpoint)."""
    chore = ScheduledChore(
        uid="friday",
        chore_name="Friday Chore",
        chore_type=ChoreType.SCHEDULED,
        rrule="FREQ=WEEKLY;BYDAY=FR",
        dtstart=datetime(2026, 1, 2, 8, 0),
        pending_period=timedelta(hours=3),
        grace_period=timedelta(hours=1),
        last_completed=datetime(2026, 3, 27, 8, 30, tzinfo=TZ),
    )
    # Skip the upcoming Friday Apr 3.
    chore.apply_occurrence_skip(datetime(2026, 3, 28, 12, 0, tzinfo=TZ), None)
    ...
    events = await _events_for(
        ...
        config_entry,
        chore,
        frozen=datetime(2026, 3, 28, 12, 0, tzinfo=TZ),
        start=datetime(2026, 3, 28, 0, 0, tzinfo=TZ),
        end=datetime(2026, 5, 1, 0, 0, tzinfo=TZ),
    )

    # Apr 3 is a hole; Apr 10 (the skip anchor, on-grid) and the following
    # Fridays are unchanged.
    assert [e.start for e in _due_events(events)] == [
        datetime(2026, 4, 10, 8, 0, tzinfo=TZ),
        datetime(2026, 4, 17, 8, 0, tzinfo=TZ),
        datetime(2026, 4, 24, 8, 0, tzinfo=TZ),
    ]
    assert _due_events(events)[0].recurrence_id == "20260410T080000"
