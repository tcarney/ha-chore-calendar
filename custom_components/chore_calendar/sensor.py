...
async def async_setup_entry(
    ...
    tracked_chore_ids: set[str] = set()

    # Add sensors for chores that already exist in storage.
    _add_new_sensors(coordinator, entry, async_add_entities, tracked_chore_ids)

    @callback
    def _async_on_coordinator_update() -> None:
        """React to coordinator updates — add/remove sensors as chores change."""
        _add_new_sensors(coordinator, entry, async_add_entities, tracked_chore_ids)
        _remove_deleted_sensors(hass, entry.entry_id, coordinator, tracked_chore_ids)
...
def _add_new_sensors(
    ...
    entry: ChoreCalendarConfigEntry,
    async_add_entities: AddEntitiesCallback,
    tracked_chore_ids: set[str],
) -> None:
    """Create sensor entities for any chores not yet tracked."""
    if coordinator.data is None:
        return

    new_chore_ids = coordinator.data.keys() - tracked_chore_ids
    if not new_chore_ids:
        return

    new_entities = [ChoreSensorEntity(coordinator, entry, chore_id) for chore_id in new_chore_ids]
    tracked_chore_ids.update(new_chore_ids)
...
def _remove_deleted_sensors(
    ...
    deleted_ids = tracked_chore_ids - coordinator.data.keys()
    if not deleted_ids:
        return

    registry = er.async_get(hass)
    for chore_id in deleted_ids:
        unique_id = f"{entry_id}_{chore_id}"
        entity_id = registry.async_get_entity_id("sensor", DOMAIN, unique_id)
        if entity_id:
            registry.async_remove(entity_id)
    tracked_chore_ids -= deleted_ids
...
class ChoreSensorEntity(CoordinatorEntity[ChoreCalendarCoordinator], SensorEntity):
    ...
    def name(self) -> str:
        ...
        return chore.chore_name if chore else self._chore_id

    @property
    def suggested_object_id(self) -> str:
        """Suggest an object_id based on the chore_id."""
    ...
    def available(self) -> bool:
        ...
        return super().available and self.coordinator.data is not None and self._chore_id in self.coordinator.data
    ...
    def _get_chore(self) -> BaseChore | None:
        ...
        if self.coordinator.data is None:
            return None
        return self.coordinator.data.get(self._chore_id)
